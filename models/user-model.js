...
module.exports={
	...
	userprofileupdate : function(data,callback){
		var sql="UPDATE `user` SET `name`=?,`email`=?,`phone`=?,`address`=?,`db`=?,`image`=? WHERE `username`=?";
		var param=[data.name,data.email,data.phone,data.address,data.db,data.image,data.username];
